int analyze_oneMCP(int run=44718) {
  ...
  TList *listI = (TList*) list->FindObject("mcpI");
  TList *list00 = (TList*) list0->FindObject("MCP0");
  TList *list11 = (TList*) list1->FindObject("MCP1");

  TH1D *MCP0_A_rmaxY = (TH1D*) list00->FindObject("MCP0_A_rmaxy");
  TH1D *MCP1_A_rmaxY = (TH1D*) list11->FindObject("MCP1_A_rmaxy");

  TH2D *fTRKxyMCP0 = (TH2D*) list0->FindObject("fTRKxyMCP0");
  TH2D *fTRKxyMCP1 = (TH2D*) list1->FindObject("fTRKxyMCP1");
  TH2D *fTRKxyMCPI = (TH2D*) listI->FindObject("fTRKxyMCPI");
